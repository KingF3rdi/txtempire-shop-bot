...
Verkauf von HugoSMP-Macro-Lizenzkeys (14d / 30d / Lifetime) über den Shop-Bot.

Keys sind offline signiert (HMAC) — kein License-Server nötig.
Aktivierung in der HugoSMP-Mod + optional Discord-Webhook-Bestätigung.

.env:
  HUGOMACRO_LICENSE_SECRET=...   # gleich wie in License.java der HugoSMP-Mods
  HUGOMACRO_WEBHOOK_URL=https://discord.com/api/webhooks/...
...
def _secret_configured() -> bool:
    return hugomacro_licensing.licensing_configured()


def _webhook_url() -> str:
    return (
        (getattr(config, "HUGOMACRO_WEBHOOK_URL", "") or "").strip()
...
def _post_webhook(title: str, description: str, color: int = 0xC8A24A) -> None:
    ...
    embed = {
        ...
        "footer": {"text": "HugoSMP Macro License"},
    }
    try:
...
def _create_license_key(plan: str, created_by: str, note: str = "") -> tuple[bool, str, str]:
    ...
    if not _secret_configured():
        return False, "HUGOMACRO_LICENSE_SECRET fehlt in .env", ""
    try:
        key = hugomacro_licensing.generate_license_key(plan)
    except Exception as e:
        return False, str(e), ""
    ok, meta, err = hugomacro_licensing.verify_own_key(key)
    if not ok:
        return False, err or "Selbstcheck fehlgeschlagen", ""
    _post_webhook(
        "HugoSMP Macro — Key erstellt",
...
async def _ensure_tables(bot: "ShopBot") -> None:
    await bot.db.db.executescript(
        """
        CREATE TABLE IF NOT EXISTS hugomacro_settings (
        ...
        CREATE TABLE IF NOT EXISTS hugomacro_keys (
...
async def _get_settings(bot: "ShopBot", guild_id: int) -> dict:
    row = await bot.db.fetchone(
        "SELECT * FROM hugomacro_settings WHERE guild_id = ?", (guild_id,)
    )
    if row:
        return dict(row)
    defaults = {
        "price_14d": float(getattr(config, "HUGOMACRO_PRICE_14D", 4.99) or 4.99),
        "price_30d": float(getattr(config, "HUGOMACRO_PRICE_30D", 9.99) or 9.99),
        "price_lifetime": float(getattr(config, "HUGOMACRO_PRICE_LIFETIME", 29.99) or 29.99),
    }
    await bot.db.db.execute(
        """
        INSERT INTO hugomacro_settings (guild_id, price_14d, price_30d, price_lifetime)
        VALUES (?, ?, ?, ?)
        """,
...
async def _update_settings(bot: "ShopBot", guild_id: int, **fields: Any) -> None:
    ...
    await bot.db.db.execute(
        f"UPDATE hugomacro_settings SET {cols} WHERE guild_id = ?", values
    )
    await bot.db.db.commit()
...
async def _next_key_number(bot: "ShopBot", guild_id: int) -> int:
    ...
    await bot.db.db.execute(
        "UPDATE hugomacro_settings SET next_key_number = ? WHERE guild_id = ?",
        (n + 1, guild_id),
    )
    await bot.db.db.commit()
    return n
...
async def _count_open_keys(bot: "ShopBot", guild_id: int, user_id: int) -> int:
    row = await bot.db.fetchone(
        """
        SELECT COUNT(*) AS cnt FROM hugomacro_keys
...
async def _create_pending_key(
    ...
    cur = await bot.db.db.execute(
        """
        INSERT INTO hugomacro_keys (guild_id, key_number, user_id, tier, price, note, status)
        VALUES (?, ?, ?, ?, ?, ?, 'pending')
        """,
...
async def _delete_key_row(bot: "ShopBot", key_id: int) -> None:
    await bot.db.db.execute("DELETE FROM hugomacro_keys WHERE id = ?", (key_id,))
    await bot.db.db.commit()


async def _set_ticket_channel(bot: "ShopBot", key_id: int, channel_id: int) -> None:
    await bot.db.db.execute(
        "UPDATE hugomacro_keys SET ticket_channel_id = ? WHERE id = ?",
        (channel_id, key_id),
    )
    await bot.db.db.commit()
...
async def _get_key_by_channel(bot: "ShopBot", channel_id: int) -> Optional[dict]:
    row = await bot.db.fetchone(
        "SELECT * FROM hugomacro_keys WHERE ticket_channel_id = ?", (channel_id,)
    )
...
async def _is_hugomacro_staff(bot: "ShopBot", interaction: discord.Interaction) -> bool:
...
async def handle_buy_hugomacro(bot: "ShopBot", interaction: discord.Interaction) -> None:
    ...
    if open_n >= 1:
        await interaction.response.send_message(
            embed=error_embed(
                "Bereits offen",
                "Du hast schon eine offene HugoSMP-Macro-Bestellung. Schließe erst das Ticket.",
            ),
            ephemeral=True,
        )
        return
...
class HugoMacroKeyPanelView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="hugomacro:buy",
        emoji="💰",
    )
    async def buy(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await handle_buy_hugomacro(self.bot, interaction)


class HugoMacroTierSelectView(discord.ui.View):
    ...
    async def _on_select(self, interaction: discord.Interaction) -> None:
        ...
        await open_hugomacro_ticket(self.bot, interaction, tier, note="")


async def open_hugomacro_ticket(
    bot: "ShopBot",
    ...
    name = f"hugomacro-{key_number:04d}-{safe}"[:100]
    ...
    try:
        channel = await guild.create_text_channel(
            ...
            reason=f"HugoSMP-Macro-Key-Ticket von {interaction.user}",
        )
    ...
    await channel.send(
        ...
        embed=embed,
        view=HugoMacroKeyTicketView(bot),
    )
    await interaction.followup.send(
...
class HugoMacroKeyTicketView(discord.ui.View):
    def __init__(self, bot: "ShopBot") -> None:
        super().__init__(timeout=None)
        self.bot = bot
    ...
    async def confirm(
        ...
        if interaction.guild is None:
            return
        if not await _is_hugomacro_staff(self.bot, interaction):
            await interaction.response.send_message(
        ...
        if not row:
            await interaction.response.send_message(
                embed=error_embed("Kein HugoSMP-Macro-Ticket"), ephemeral=True
            )
            return
        ...
        if not _secret_configured():
            await interaction.response.send_message(
                embed=error_embed(
                    "Secret fehlt",
                    "HUGOMACRO_LICENSE_SECRET in .env setzen (gleich wie in der HugoSMP-Mod).",
                ),
                ephemeral=True,
            )
            return
        ...
        if buyer is not None:
            try:
                await buyer.send(
                    embed=success_embed(
                        "💰 Dein HugoSMP Macro License Key",
                        f"Plan: **{TIER_LABELS[row['tier']]}**\n\n"
                        f"```\n{license_key}\n```\n"
                        "In **HugoSMP Macro** unter Lizenzkey einfügen → **Aktivieren**.\n"
...
# ── Slash-Commands: nur die Gruppe /hugomacro (Discord-Limit: 100 Top-Level-Befehle) ──

class HugoMacroKeysCog(commands.Cog):
    def __init__(self, bot: "ShopBot") -> None:
        self.bot = bot

    async def hugomacrosetup(
        self,
        interaction: discord.Interaction,
    ...
    async def hugomacropanel(
        ...
        msg = await target.send(
            embed=_panel_embed(settings), view=HugoMacroKeyPanelView(self.bot)
        )
        await interaction.followup.send(
            embed=success_embed(
                "HugoSMP-Macro-Panel gepostet", f"In {target.mention}: {msg.jump_url}"
            ),
            ephemeral=True,
        )

    async def hugomacrogen(
        self,
        interaction: discord.Interaction,
        ...
        if not _secret_configured():
            await interaction.response.send_message(
                embed=error_embed(
                    "Secret fehlt",
                    "In `.env` setzen:\n"
                    "`HUGOMACRO_LICENSE_SECRET=...`\n"
                    "Optional: `HUGOMACRO_WEBHOOK_URL=...`\n"
            ...
            try:
                await user.send(
                    embed=success_embed(
                        "Dein HugoSMP Macro License Key",
                        f"Plan: **{plan.name}**\n```\n{key_or_err}\n```\n"
                        "Im Spiel Menü-Taste → Key einfügen → Aktivieren (HWID-Bindung).",
                    )
                )
            except discord.HTTPException:
                await interaction.followup.send(
    ...
    async def hugomacroplans(self, interaction: discord.Interaction) -> None:
        assert interaction.guild is not None
        settings = await _get_settings(self.bot, interaction.guild.id)
        lines = []
        for tier in TIER_ORDER:
    ...
    async def hugomacrobuy(self, interaction: discord.Interaction) -> None:
        await handle_buy_hugomacro(self.bot, interaction)

    async def hugomacrostatus(self, interaction: discord.Interaction) -> None:
        await interaction.response.defer(ephemeral=True)
        ...
        if secret_ok:
            ...
            try:
                key = hugomacro_licensing.generate_license_key("14d")
                ok, _, err = hugomacro_licensing.verify_own_key(key)
                sample = f"\nSelftest: {'✅ OK ' + key[:26] + '…' if ok else '⚠️ ' + (err or '?')}"
            except Exception as e:
                sample = f"\nSelftest: ⚠️ {e}"
        embed = (success_embed if secret_ok else warn_embed)(
            "HugoSMP Macro Offline-Lizenz",
            f"Secret: {'✅ gesetzt' if secret_ok else '⚠️ HUGOMACRO_LICENSE_SECRET fehlt'}\n"
            f"Webhook: {'✅ gesetzt' if wh else '⚠️ HUGOMACRO_WEBHOOK_URL fehlt'}"
            f"{sample}\n\n"
            "Kein License-Server nötig — Keys lokal signiert, Bestätigung per Webhook.",
        )
        await interaction.followup.send(embed=embed, ephemeral=True)

    # Gruppe /hugomacro … ruft die Methoden oben auf (keine eigenen Top-Level-Befehle).
    hugomacro = app_commands.Group(
        name="hugomacro",
    ...
    @hugomacro.command(name="buy", description="HugoSMP Macro kaufen")
    async def hugomacro_buy(self, interaction: discord.Interaction) -> None:
        await self.hugomacrobuy(interaction)

    @hugomacro.command(name="panel", description="Kauf-Panel posten (Staff)")
    @app_commands.describe(channel="Ziel-Channel")
    @app_commands.default_permissions(manage_guild=True)
    async def hugomacro_panel(
        self,
        interaction: discord.Interaction,
        channel: discord.TextChannel | None = None,
    ) -> None:
        await self.hugomacropanel(interaction, channel)
    ...
    @hugomacro.command(name="gen", description="Key erzeugen (Staff)")
    @app_commands.describe(plan="Plan", user="DM an User", note="Notiz")
    @app_commands.choices(
        plan=[
            app_commands.Choice(name="14 Tage", value="14d"),
            app_commands.Choice(name="30 Tage", value="30d"),
            app_commands.Choice(name="Lifetime", value="lifetime"),
        ]
    )
    @app_commands.default_permissions(manage_guild=True)
    async def hugomacro_gen(
        self,
        interaction: discord.Interaction,
        plan: app_commands.Choice[str],
        user: discord.User | None = None,
        ...
        await self.hugomacrogen(interaction, plan, user, note)

    @hugomacro.command(name="status", description="Lizenz / Webhook Status")
    async def hugomacro_status(self, interaction: discord.Interaction) -> None:
        await self.hugomacrostatus(interaction)


async def setup(bot: "ShopBot") -> None:
    await _ensure_tables(bot)
    await bot.add_cog(HugoMacroKeysCog(bot))
    print(
        "[HugoMacro] Cog geladen — /hugomacro panel|setup|gen|plans|buy|status"
    )
