...
Verkauf von Lizenzkeys für "Quick Invsee" (Minecraft-Mod) direkt über diesen Bot -
...
Preise und Custom-IDs (getrenntes Produkt, getrennter Lizenz-Secret):
  ...
  - Staff bestätigt im Ticket mit einem Klick ("✅ Bestätigen") ->
    ...
    Lizenzkey und schickt ihn dem Kunden per DM. Die Mod bindet ihn
    automatisch an das Geraet des Kunden, sobald er ihn dort zum
    ersten Mal eintraegt. Kein manueller Schritt außer dem einen
    Klick nötig.
  - Alternativ: `/qikey generate` erzeugt (für Staff) sofort einen
...
Braucht: QUICKINVSEE_LICENSE_SECRET in .env (siehe .env.example) -
muss exakt mit SECRET in License.java der Quick-Invsee-Mod
übereinstimmen.
Eigene Tabellen (qi_settings, qi_keys) - keine Änderung an db/database.py
nötig.
"""
...
from utils import quickinvsee_licensing as qilic
...
TIER_ORDER = (qilic.TIER_14D, qilic.TIER_30D, qilic.TIER_LIFETIME)
...
async def _ensure_tables(bot: "ShopBot") -> None:
    await bot.db.db.executescript(
        """
        CREATE TABLE IF NOT EXISTS qi_settings (
        ...
        CREATE TABLE IF NOT EXISTS qi_keys (
...
async def _get_settings(bot: "ShopBot", guild_id: int) -> dict:
    row = await bot.db.fetchone(
        "SELECT * FROM qi_settings WHERE guild_id = ?", (guild_id,)
    )
    if row:
        return dict(row)
    defaults = {
        "price_14d": config.QUICKINVSEE_PRICE_14D,
        "price_30d": config.QUICKINVSEE_PRICE_30D,
        "price_lifetime": config.QUICKINVSEE_PRICE_LIFETIME,
    }
    await bot.db.db.execute(
        """
        INSERT INTO qi_settings (guild_id, price_14d, price_30d, price_lifetime)
        VALUES (?, ?, ?, ?)
        """,
...
async def _update_settings(bot: "ShopBot", guild_id: int, **fields) -> None:
    ...
    await bot.db.db.execute(
        f"UPDATE qi_settings SET {cols} WHERE guild_id = ?", values
    )
    await bot.db.db.commit()
...
async def _next_key_number(bot: "ShopBot", guild_id: int) -> int:
    ...
    await bot.db.db.execute(
        "UPDATE qi_settings SET next_key_number = ? WHERE guild_id = ?",
        (n + 1, guild_id),
    )
    await bot.db.db.commit()
    return n
...
async def _count_open_keys(bot: "ShopBot", guild_id: int, user_id: int) -> int:
    row = await bot.db.fetchone(
        """
        SELECT COUNT(*) AS cnt FROM qi_keys
...
async def _create_pending_key(
    ...
    cur = await bot.db.db.execute(
        """
        INSERT INTO qi_keys (guild_id, key_number, user_id, hwid, tier, price, note, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')
        """,
...
async def _get_key_by_channel(bot: "ShopBot", channel_id: int) -> Optional[dict]:
    row = await bot.db.fetchone(
        "SELECT * FROM qi_keys WHERE ticket_channel_id = ?", (channel_id,)
    )
    return dict(row) if row else None


async def _get_key_by_id(bot: "ShopBot", key_id: int) -> Optional[dict]:
    row = await bot.db.fetchone("SELECT * FROM qi_keys WHERE id = ?", (key_id,))
    return dict(row) if row else None
...
async def _delete_key_row(bot: "ShopBot", key_id: int) -> None:
    await bot.db.db.execute("DELETE FROM qi_keys WHERE id = ?", (key_id,))
    await bot.db.db.commit()


async def _set_ticket_channel(bot: "ShopBot", key_id: int, channel_id: int) -> None:
    await bot.db.db.execute(
        "UPDATE qi_keys SET ticket_channel_id = ? WHERE id = ?", (channel_id, key_id)
    )
    await bot.db.db.commit()
...
async def _mark_confirmed(bot: "ShopBot", key_id: int, license_key: str, staff_id: int) -> None:
    ...
    if shop_api.relay_webhook_url:
        row = await bot.db.fetchone("SELECT price FROM qi_keys WHERE id = ?", (key_id,))
...
async def _list_recent_keys(bot: "ShopBot", guild_id: int, limit: int = 15) -> list[dict]:
    rows = await bot.db.fetchall(
        """
        SELECT * FROM qi_keys WHERE guild_id = ?
...
def _panel_embed(settings: dict) -> discord.Embed:
    ...
    for tier in TIER_ORDER:
        ...
        lines.append(f"**{qilic.TIER_LABELS[tier]}** — {price_txt}")
    embed = base_embed(
        "🔍 Quick Invsee — Lizenzkey",
        "Minecraft-Mod (Fabric): /invsee per Hotkey auf den anvisierten Spieler, "
        "Gegner-Anzeige + Auto-Invsee bei /rtpqueue.\n\n"
        ...
        "sich automatisch an dein Gerät, sobald du ihn ingame einträgst (/quickinvsee key <KEY> oder Mod-Menü).",
    )
    return embed
...
async def handle_buy_key(bot: "ShopBot", interaction: discord.Interaction) -> None:
    if interaction.guild is None:
        ...
        return
    if not qilic.licensing_configured():
        await interaction.response.send_message(
            embed=error_embed(
                "Noch nicht eingerichtet",
                "QUICKINVSEE_LICENSE_SECRET ist noch nicht gesetzt (Staff).",
            ),
            ephemeral=True,
        )
        return
...
async def handle_reset_hwid(bot: "ShopBot", interaction: discord.Interaction) -> None:
    ...
    if row.get("license_key"):
        ok, old_payload, _err = qilic.verify_own_key(row["license_key"])
        if ok and old_payload:
            issued = old_payload.get("issued")

    new_key = qilic.generate_license_key(None, tier=row["tier"], issued=issued)
    expires = qilic.tier_to_expiry(row["tier"], issued)
    ...
    try:
        await interaction.user.send(
            embed=success_embed(
                "🔄 Neuer Quick Invsee Key (HWID-Reset)",
                f"Laufzeit: **{qilic.describe_tier(row['tier'], expires)}**\n\n"
                f"```\n{new_key}\n```\n"
                "Ingame mit `/quickinvsee key <KEY>` (oder im Mod-Menü) eintragen - bindet sich "
...
class QuickinvseeKeyPanelView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="quickinvsee:reset_hwid",
        emoji="🔄",
    )
...
class TierSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        # des Kunden, sobald er ihn zum ersten Mal ingame eintraegt.
...
async def _create_key_ticket_channel(
    ...
    *,
    ...
    qi_settings = await _get_settings(bot, guild.id)
    price = _price_for(qi_settings, tier)

    category_id = settings.get("ticket_category_id")
    category = guild.get_channel(int(category_id)) if category_id else None
    if category is not None and not isinstance(category, discord.CategoryChannel):
        category = None
    staff_role = await _resolve_support_role(bot, guild, qi_settings)
    me = guild.me
    if me is None:
        await interaction.followup.send(
    ...
    name = f"qikey-{key_number:04d}-{safe}"[:100]
    ...
    try:
        channel = await guild.create_text_channel(
            ...
            reason=f"Quick-Invsee-Key-Ticket von {interaction.user}",
        )
    ...
    embed = base_embed(
        ...
        f"Laufzeit: **{qilic.TIER_LABELS[tier]}**\n"
        f"Preis: **{price_txt}**\n"
    ...
    await channel.send(
        ...
        embed=embed,
        view=QuickinvseeKeyTicketView(bot),
    )
    await interaction.followup.send(
...
async def _resolve_support_role(
    bot: "ShopBot", guild: discord.Guild, qi_settings: Optional[dict] = None
) -> Optional[discord.Role]:
    """Eigene Quick-Invsee-Support-Rolle, falls gesetzt (/qikey setup support_role:@...)
    - sonst Fallback auf die normale Shop-Staff-Rolle (/setup)."""
    if qi_settings is None:
        qi_settings = await _get_settings(bot, guild.id)
    role_id = qi_settings.get("support_role_id")
    if role_id:
...
async def _is_quickinvsee_staff(bot: "ShopBot", interaction: discord.Interaction) -> bool:
    """Wie is_staff(), aber die eigene Quick-Invsee-Support-Rolle zählt
    zusätzlich zur normalen Shop-Staff-Rolle."""
    user = interaction.user
    if isinstance(user, discord.Member) and user.guild_permissions.administrator:
        return True
    assert interaction.guild is not None
    qi_settings = await _get_settings(bot, interaction.guild.id)
    role_id = qi_settings.get("support_role_id")
...
class QuickinvseeKeyTicketView(discord.ui.View):
    def __init__(self, bot: "ShopBot") -> None:
        super().__init__(timeout=None)
        self.bot = bot
    ...
    async def confirm(
        ...
        if interaction.guild is None:
            return
        if not await _is_quickinvsee_staff(self.bot, interaction):
            await interaction.response.send_message(
        ...
        if not qilic.licensing_configured():
            await interaction.response.send_message(
                embed=error_embed(
                    "Nicht eingerichtet", "QUICKINVSEE_LICENSE_SECRET fehlt in .env."
                ),
                ephemeral=True,
            )
            return
        ...
        license_key = qilic.generate_license_key(row["hwid"] or None, tier=row["tier"])
        expires = qilic.tier_to_expiry(row["tier"])
        ...
        if buyer is not None:
            try:
                await buyer.send(
                    embed=success_embed(
                        "🔑 Dein Quick Invsee Key",
                        f"Laufzeit: **{qilic.describe_tier(row['tier'], expires)}**\n\n"
                        f"```\n{license_key}\n```\n"
                        "Ingame mit `/quickinvsee key <KEY>` (oder im Mod-Menü) eintragen. "
            ...
            try:
                await buyer.send(
                    embed=warn_embed(
                        ...
                        "Deine Quick-Invsee-Key-Bestellung wurde abgelehnt "
    ...
    async def close(
        ...
        if not row:
            ...
            return
        staff = await _is_quickinvsee_staff(self.bot, interaction)
...
class QuickinvseeKeysCog(commands.Cog):
    def __init__(self, bot: "ShopBot") -> None:
        self.bot = bot

    qikey = app_commands.Group(
        name="qikey",
        description="Quick-Invsee-Lizenzkeys verwalten (Staff)",
        default_permissions=discord.Permissions(manage_guild=True),
    )

    @qikey.command(
        name="setup",
        description="Preise für Quick-Invsee-Keys setzen (Staff)",
    )
    @app_commands.describe(
        ...
        support_role=(
            "Eigene Support-Rolle für Quick-Invsee-Ticket (sieht Tickets, darf "
    ...
    async def qikeysetup(
        ...
        secret_ok = qilic.licensing_configured()
        ...
        await interaction.response.send_message(
            embed=success_embed(
                "Quick-Invsee Key-Einstellungen",
                f"14 Tage: **{format_price(_price_for(settings, qilic.TIER_14D))}**\n"
                f"30 Tage: **{format_price(_price_for(settings, qilic.TIER_30D))}**\n"
                f"Lifetime: **{format_price(_price_for(settings, qilic.TIER_LIFETIME))}**\n"
                f"{role_line}\n\n"
                + (
                    "✅ QUICKINVSEE_LICENSE_SECRET ist gesetzt."
                    if secret_ok
                    else "⚠️ QUICKINVSEE_LICENSE_SECRET fehlt noch in der .env "
                    "(muss mit SECRET in License.java der Mod übereinstimmen)."
                ),
            ),
            ephemeral=True,
        )
    ...
    @qikey.command(
        name="panel",
        description="Kauf-Panel für Quick-Invsee-Keys posten (Staff)",
    )
    @app_commands.describe(channel="Ziel-Channel (Standard: aktuell)")
    @app_commands.default_permissions(manage_guild=True)
    async def qikeypanel(
        self,
        interaction: discord.Interaction,
        ...
        msg = await target.send(
            embed=_panel_embed(settings), view=QuickinvseeKeyPanelView(self.bot)
        )
        await interaction.followup.send(
    ...
    @qikey.command(name="generate", description="Sofort einen gültigen Key erzeugen (Staff)")
    @app_commands.describe(
        tier="Laufzeit",
    ...
    @app_commands.choices(
        tier=[
            app_commands.Choice(name="Lifetime", value=qilic.TIER_LIFETIME),
            app_commands.Choice(name="14 Tage", value=qilic.TIER_14D),
            app_commands.Choice(name="30 Tage", value=qilic.TIER_30D),
        ]
    )
    async def qikey_generate(
        self,
        interaction: discord.Interaction,
        ...
        if not qilic.licensing_configured():
            await interaction.response.send_message(
                embed=error_embed(
                    "Nicht eingerichtet",
                    "QUICKINVSEE_LICENSE_SECRET fehlt in der .env.",
                ),
                ephemeral=True,
            )
            return
        await interaction.response.defer(ephemeral=True)
        expires = qilic.tier_to_expiry(tier.value)
        license_key = qilic.generate_license_key(hwid or None, tier=tier.value)
        ...
        if member is not None:
            try:
                await member.send(
                    embed=success_embed(
                        "🔑 Dein Quick Invsee Key",
                        f"Laufzeit: **{qilic.describe_tier(tier.value, expires)}**\n\n"
                        f"```\n{license_key}\n```\n"
                        "Ingame mit `/quickinvsee key <KEY>` (oder im Mod-Menü) eintragen.",
                    )
                )
    ...
    @qikey.command(name="list", description="Zuletzt ausgestellte Keys anzeigen (Staff)")
    async def qikey_list(self, interaction: discord.Interaction) -> None:
        assert interaction.guild is not None
        ...
        for r in rows:
            ...
            lines.append(
                f"**#{r['key_number']}** · {qilic.TIER_LABELS.get(r['tier'], r['tier'])} "
                f"· `{r['status']}` · {who}"
            )
        await interaction.response.send_message(
            embed=success_embed("Letzte Quick-Invsee-Keys", "\n".join(lines[:15])),
            ephemeral=True,
        )
...
async def setup(bot: "ShopBot") -> None:
    ...
    await bot.add_cog(QuickinvseeKeysCog(bot))
