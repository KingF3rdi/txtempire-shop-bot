"""
gputweaks_licensing.py
========================

Signiert Lizenzkeys für "y3zz GPU Tweaks" - byte-kompatibel mit licensing.py
in der y3zz-GPU-Tweaks-App. Bot und App benutzen exakt denselben Algorithmus
(HMAC-SHA256 über ein kompaktes 21-Byte-Binärpaket, Base32-kodiert und in
5er-Gruppen dargestellt - siehe licensing.py der App, Abschnitt "KEY-FORMAT"
für die genaue Byte-Aufteilung); ein hier erzeugter Key wird von der App
...
WICHTIG: config.GPUTWEAKS_LICENSE_SECRET (aus der .env) MUSS exakt dem Wert
entsprechen, der in der App bei licensing.py -> LICENSE_SECRET eingetragen
...
def licensing_configured() -> bool:
    return bool(getattr(config, "GPUTWEAKS_LICENSE_SECRET", "").strip())
...
def generate_license_key(
    ...
    Braucht config.GPUTWEAKS_LICENSE_SECRET (siehe licensing_configured()).

    `hwid`: None/leer erzeugt einen NICHT gebundenen Key - so wird er sofort
    beim Kauf ausgegeben, ohne vorher die Hardware-ID abzufragen. Die App
    ...
    secret = config.GPUTWEAKS_LICENSE_SECRET.strip()
    if not secret:
        raise RuntimeError(
            "GPUTWEAKS_LICENSE_SECRET ist nicht gesetzt (siehe .env)."
        )
    if issued is None:
...
def verify_own_key(key: str) -> Tuple[bool, Optional[dict], str]:
    """Nur zu Debug-/Testzwecken (z.B. /gtkey generate zeigt danach eine
    Selbstprüfung) - prüft NICHT die Hardware-ID, weil der Bot keine hat.
    Die eigentliche Prüfung passiert in der App (licensing.py). Gibt im
    Payload-dict "hwid" (oder None), "tier", "issued", "expires" zurück."""
    secret = config.GPUTWEAKS_LICENSE_SECRET.strip()
    if not secret:
        return False, None, "GPUTWEAKS_LICENSE_SECRET nicht gesetzt."
