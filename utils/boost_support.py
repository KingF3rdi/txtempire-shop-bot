"""Gemeinsames Support-System für Account-Buy und Tier-Buy.

Beide Shops teilen sich EINE Support-Rolle plus eine Liste einzelner
Support-Mitglieder, die auch OHNE diese Rolle Zugriff auf die Tickets haben
(z.B. Admins, die die öffentliche Rolle nicht tragen wollen).
"""
...
async def ensure_table(bot: "ShopBot") -> None:
    await bot.db.db.executescript(
        """
        CREATE TABLE IF NOT EXISTS boost_support_settings (
...
async def _get_settings(bot: "ShopBot", guild_id: int) -> dict:
    row = await bot.db.fetchone(
        "SELECT * FROM boost_support_settings WHERE guild_id = ?", (guild_id,)
    )
    if row:
        return dict(row)
...
async def set_staff_role_id(bot: "ShopBot", guild_id: int, role_id: int) -> None:
    await bot.db.db.execute(
        """
        INSERT INTO boost_support_settings (guild_id, staff_role_id) VALUES (?, ?)
...
async def add_extra_staff(bot: "ShopBot", guild_id: int, user_id: int) -> None:
    ...
    await bot.db.db.execute(
        """
        INSERT INTO boost_support_settings (guild_id, extra_staff_ids) VALUES (?, ?)
...
async def is_boost_staff(bot: "ShopBot", interaction: discord.Interaction) -> bool:
...
async def boost_staff_role(bot: "ShopBot", guild: discord.Guild) -> discord.Role | None:
