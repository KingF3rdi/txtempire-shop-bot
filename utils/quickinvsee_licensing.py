"""
quickinvsee_licensing.py
==========================

Signiert Lizenzkeys für "Quick Invsee" (Minecraft-Mod) - byte-kompatibel mit
License.java in der Mod. Bot und Mod benutzen exakt denselben Algorithmus
(HMAC-SHA256 über ein kompaktes 21-Byte-Binärpaket, Base32-kodiert und in
5er-Gruppen dargestellt - siehe License.java der Mod, Abschnitt "KEY-FORMAT"
für die genaue Byte-Aufteilung); ein hier erzeugter Key wird von der Mod
offline geprüft, ohne dass irgendein Server erreichbar sein muss.

Eigenständiges Geheimnis, getrennt von MOUSETWEAKS_LICENSE_SECRET - Keys der
beiden Produkte sind absichtlich nicht wechselseitig gültig.

WICHTIG: config.QUICKINVSEE_LICENSE_SECRET (aus der .env) MUSS exakt dem Wert
entsprechen, der in der Mod bei License.java -> SECRET eingetragen
ist. Ändert sich einer der beiden Werte, werden alle bisher ausgestellten
Keys für die jeweils andere Seite ungültig.
...
def licensing_configured() -> bool:
    return bool(getattr(config, "QUICKINVSEE_LICENSE_SECRET", "").strip())
...
def generate_license_key(
    ...
    Braucht config.QUICKINVSEE_LICENSE_SECRET (siehe licensing_configured()).

    `hwid`: None/leer erzeugt einen NICHT gebundenen Key - so wird er sofort
    beim Kauf ausgegeben, ohne vorher die Hardware-ID abzufragen. Die Mod
    bindet ihn automatisch an das Geraet des Kunden beim ersten Eintragen.
    ...
    secret = config.QUICKINVSEE_LICENSE_SECRET.strip()
    if not secret:
        raise RuntimeError(
            "QUICKINVSEE_LICENSE_SECRET ist nicht gesetzt (siehe .env)."
        )
    if issued is None:
...
def verify_own_key(key: str) -> Tuple[bool, Optional[dict], str]:
    """Nur zu Debug-/Testzwecken (z.B. /qikey generate zeigt danach eine
    Selbstprüfung) - prüft NICHT die Hardware-ID, weil der Bot keine hat.
    Die eigentliche Prüfung passiert in der Mod (License.java). Gibt im
    Payload-dict "hwid" (oder None), "tier", "issued", "expires" zurück."""
    secret = config.QUICKINVSEE_LICENSE_SECRET.strip()
    if not secret:
        return False, None, "QUICKINVSEE_LICENSE_SECRET nicht gesetzt."
